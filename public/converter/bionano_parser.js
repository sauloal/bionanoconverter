...
bionano_parser.prototype.report_xmap    = function(self, clbk) {
  ...
  for ( var chromnum in chroms ) {
    ...
    console.log('report_xmap :: chrom_name', chrom_name);
    console.log('report_xmap :: chrom_id  ', chrom_id  );
    
    var cdata      = data[chrom_id];
    console.log('report_xmap :: cdata     ', cdata       );
    console.log('report_xmap :: cdatalen  ', cdata.length);
    
    outdata_wig.push( '#browser position '+chrom_name+':0-'+(chrom_size - 1) );
    outdata_wig.push( '#track type=wiggle_0 name="xmap" description="BioNano Genomics - XMAP Nicking Pattern" visibility=full autoScale=off viewLimits=0.0:1.0 color=0,255,00 altColor=255,0,0 alwaysZero=on graphType=bar smoothingWindow=off priority=10' );
    outdata_wig.push( 'variableStep chrom='+chrom_name+' span='+nick_sites[Object.keys(nick_sites)[0]][1] );
    ...
    for ( var rowid in cdata ) {
      ...
      */
      ...
      /*
      var pieces = Alignment.substring(1,Alignment.length-1).split(")(");
      console.log('report_xmap :: pieces      ', pieces.length, pieces);
